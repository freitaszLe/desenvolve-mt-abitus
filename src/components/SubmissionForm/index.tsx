...
// O schema não valida mais o 'foto', pois o gerenciamos manualmente.
...
// O tipo de dados que o formulário manipula (sem a foto)
...
const SubmissionForm = ({ personName, ocorrenciaId, onClose, onSuccess }: SubmissionFormProps) => {
  ...
  // CORREÇÃO: Pegamos o 'register' que estava faltando
  ...
  const handleFormSubmit = async (data: SubmissionFormData) => {
    ...
    try {
      // Montamos o FormData manualmente com os arquivos do nosso estado
      const formData = new FormData();
      selectedFiles.forEach(file => {
        formData.append('files', file);
      });

      // Passamos o formData e os outros dados para a API
      await postNovaInformacao(ocorrenciaId, data, formData);
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50 p-4" onClick={onClose} role="dialog" aria-modal="true">
      <div className="bg-dark-card border border-dark-border rounded-lg shadow-xl w-full max-w-lg p-6 md:p-8" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
          <div>
            ...
            <textarea id="observacoes" {...register("observacoes")} className="w-full bg-gray-cyber border border-dark-border rounded-md p-2 text-text-light focus:outline-none focus:ring-2 focus:ring-neon-red focus:border-transparent transition" rows={4} placeholder="Ex: Foi visto(a) na Praça Central..."/>
          ...
          <div>
            ...
            <input type="text" id="localizacao" {...register("localizacao")} className="w-full bg-gray-cyber border border-dark-border rounded-md p-2 text-text-light focus:outline-none focus:ring-2 focus:ring-neon-red focus:border-transparent transition" placeholder="Ex: Cuiabá, MT, Bairro Centro..."/>
          ...
          <div>
            ...
            <input type="file" id="fotos" accept="image/*" multiple onChange={handleFileChange} className="w-full text-sm text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-900/50 file:text-blue-300 hover:file:bg-blue-900 transition"/>
          </div>
